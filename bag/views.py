...
def add_to_bag(request, item_id):
    """ Adds a specific quantity of the product to shopping basket """
    ...
    if size:
        if item_id in list(bag.keys()):
            ...
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f'Added {size.upper()} {product.name} to your basket')
    else:
        if item_id in list(bag.keys()):
...
def adjust_bag(request, item_id):
    """Adjusts the quantity of the product by the specified amount"""
...
def remove_from_bag(request, item_id):
    """Removes the item from the basket entirely"""
